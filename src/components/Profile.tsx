...
const Profile: React.FC<ProfileProps> = ({ user, updateUser, onLogout, onLoginClick, language, onLanguageChange, onSetupServer, customAppName, setCustomAppName, themeColor, onThemeChange, buttonColor, onButtonColorChange, onBack }) => {
          ...
          <div className="relative w-24 h-24 mx-auto mb-4">
            <div className="w-24 h-24 bg-gradient-to-br from-zinc-800 to-zinc-950 rounded-full flex items-center justify-center text-white shadow-2xl border-4 border-white overflow-hidden">
               {user.avatar ? (
                 ...
                 <span className="text-3xl font-black">{(user.name || 'U')[0]?.toUpperCase() || 'U'}</span>
               )}
            </div>
