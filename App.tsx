  ...
  return (
    <div className={`min-h-screen ${themeColor} flex flex-col md:flex-row font-sans text-zinc-900`}>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        ...
        <main className={`flex-1 overflow-y-auto p-0 md:p-8 ${themeColor}`}>
          <div className="max-w-7xl mx-auto w-full">
            {activeTab === 'warpers' && <Warpers user={user} language={language} buttonColor={buttonColor} />}
            ...
            }} themeColor={themeColor} onThemeChange={handleThemeChange} buttonColor={buttonColor} onButtonColorChange={handleButtonColorChange} onBack={() => setActiveTab('warpers')} />}
          </div>
        </main>
      </div>
      ...
      {showAuthModal && (
          <div className="fixed inset-0 bg-black/60 z-[120] flex items-center justify-center p-4 backdrop-blur-sm auth-modal">
              <div className="relative w-full max-w-sm">
                  ...
                  <AuthScreen onLogin={u => { setUser(u); localStorage.setItem('warper_active_user', JSON.stringify(u)); setShowAuthModal(false); window.location.reload(); }} language={language} t={t} isOnline={isOnline} isModal={true} buttonColor={buttonColor} themeColor={themeColor} />
              </div>
          </div>
      )}
    </div>
...
const AuthScreen: React.FC<{ onLogin: (u: User) => void; language: 'ta' | 'en'; t: any; isOnline: boolean; isModal?: boolean; buttonColor: string; themeColor: string }> = ({ onLogin, t, isModal, buttonColor, themeColor }) => {
        ...
        if (mode === 'REGISTER') {
           ...
           if (error) {
               if (error.message.toLowerCase().includes('phone') || error.message.toLowerCase().includes('provider')) {
                   ...
                   alert(t.regSuccessPhoneNote);
               } else {
                   throw error;
               }
           } else {
               alert(t.regSuccessLoginNow);
           }
           setMode('LOGIN');
        } else if (mode === 'LOGIN') {
